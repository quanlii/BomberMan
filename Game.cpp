...
void GameBomberMan::TwoPlayersMode()
{
	...
	while (1)
	{
		if (_kbhit())
		{
			char ch = _getch();
			if (ch == 72 || ch == 80 || ch == 75 || ch == 77) player1->Move(map, ch, bomb1);
			else if (ch == 'w' || ch == 'a' || ch == 's' || ch == 'd') player2->Move(map, ch, bomb2);
			else if (ch == 13) player1->SetBomb(bomb1, map);
			else if (ch == ' ') player2->SetBomb(bomb2, map);
			else if (ch == 'p') _getch();
		}
